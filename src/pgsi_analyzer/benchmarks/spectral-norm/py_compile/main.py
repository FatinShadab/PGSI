...
@measure_energy_to_csv(n=get_measurement_runs("spectral-norm"), csv_filename="spectral_norm_pycompile")
def run_energy_benchmark(matrix: List[List[int]], iterations=10) -> None:
    spectral_norm(matrix, iterations)

@measure_time_to_csv(n=get_measurement_runs("spectral-norm"), csv_filename="spectral_norm_pycompile")
