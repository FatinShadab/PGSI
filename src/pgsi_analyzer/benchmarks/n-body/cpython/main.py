...
@measure_time_to_csv(n=get_measurement_runs("nbody"), csv_filename="nbody_cpython")
...
@measure_energy_to_csv(n=get_measurement_runs("nbody"), csv_filename="nbody_cpython")
...
if __name__ == "__main__":
    nbody_params = __default__["nbody"]
    bodies = [
        Body(body["mass"], body["position"], body["velocity"])
        for body in nbody_params.get("bodies", [])
    ]

    # Run the simulation
    dt = nbody_params.get("dt", 0.01)
    num_steps = nbody_params.get("time_steps", 100)
