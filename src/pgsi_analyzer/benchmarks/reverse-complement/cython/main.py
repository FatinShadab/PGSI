...
@measure_energy_to_csv(n=get_measurement_runs("reverse_complement"), csv_filename="reverse_complement_cython")
def run_energy_benchmark(dna_sequence: str) -> None:
    reverse_complement(dna_sequence.encode("utf-8"))
    
@measure_time_to_csv(n=get_measurement_runs("reverse_complement"), csv_filename="reverse_complement_cython")
